...
FIDELITY_REPORT = GENERATED / "mt540-mt541-sr2026-rule-fidelity.md"

PHASE_5B_FIDELITY = {
    "MT540": {"exact": 7, "partial": 7, "unsupported": 4},
    "MT541": {"exact": 8, "partial": 8, "unsupported": 4},
}
PHASE_5B_UNSUPPORTED = {
...
def _review_entry(
    ...
    lines = [
        ...
        f"| Occurrence scope | `{', '.join(rule.get('occurrenceScopes') or ['NONE'])}` |",
...
def render_rule_fidelity(payload: dict[str, Any] | None = None) -> str:
    payload = payload or load()
    guides = sources(payload)
    lines = [
        ...
        "## Counts",
        "",
        "| Message | Total | Exact before | Exact after | Partial before | Partial after | "
        "Unsupported before | Unsupported after |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for guide in guides:
        before = PHASE_5B_FIDELITY[guide.message_type]
        exact = len(guide.by_fidelity(RuleFidelity.EXACT))
        partial = len(guide.by_fidelity(RuleFidelity.PARTIAL))
        unsupported = len(guide.by_fidelity(RuleFidelity.UNSUPPORTED))
        lines.append(
            f"| `{guide.message_type}` | {len(guide.rules())} | {before['exact']} | {exact} | "
            f"{before['partial']} | {partial} | {before['unsupported']} | {unsupported} |"
        )
    lines += [
        "",
    ...
    for guide in guides:
        rules = {item["sourceRuleId"]: item for item in guide.rules()}
        for rule_id in sorted(PHASE_5B_UNSUPPORTED[guide.message_type]):
            rule = rules[rule_id]
            residual = "; ".join(rule["residual"]) or "none"
            scope = ", ".join(rule.get("occurrenceScopes") or ["NONE"])
            lines.append(
                f"| `{guide.message_type}` | `{rule_id}` | `{rule['fidelity']}` | "
                f"`{scope}` | {residual} |"
            )
    lines += [
        "",
    ...
    for guide in guides:
        for rule in guide.rules():
            if rule["fidelity"] == RuleFidelity.EXACT.value:
                continue
            remaining += 1
            residual = "; ".join(rule["residual"]) or rule["reason"] or "none recorded"
            lines.append(
                f"| `{guide.message_type}` | `{rule['sourceRuleId']}` | "
                f"`{rule['fidelity']}` | `{rule['template']}` | {residual} |"
            )
    if remaining == 0:
        lines.append("| `NONE` | `-` | `-` | `-` | `-` |")
    lines += [
        ...
        "- `UNSUPPORTED` is zero after Phase 5C, but partial candidates still remain where "
        "the source requires component scope, data-source-scheme exceptions, paired-code "
        "semantics, or different-occurrence relationships.",
        "- No candidate is stronger than the source; a partial candidate is weaker and "
        "records what it leaves out.",
...
def rendered() -> dict[Path, str]:
    ...
    documents = {
        ...
        FIDELITY_REPORT: render_rule_fidelity(payload),
    }
