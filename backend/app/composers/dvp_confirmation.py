...
class DvpConfirmationComposer(MessageComposer):
    def compose(self, scenario: SettlementScenario, profile: ClientProfile) -> CompositionResult:
        del profile
        if scenario.message_type not in {MessageType.MT545, MessageType.MT547}:
            raise ValueError("The DVP confirmation composer supports MT545 and MT547")
        required = (
            ...
            scenario.confirmation.settled_amount,
            scenario.confirmation.settlement_result,
        )
        if any(value is None for value in required):
            raise ValueError("Composer received an incomplete MT545 scenario")
        ...
        add(
            ...
            "ESTT",
            (
                f"{scenario.settlement.currency}"
                f"{swift_decimal(scenario.confirmation.settled_amount, 2)}"
            ),
